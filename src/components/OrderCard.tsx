...
export function OrderCard({ order, isAdmin, className, onDelete }: { order: Expense; isAdmin: boolean; className?: string; onDelete?: () => void }) {
    ...
    <div className={`card-soft rounded-xl border border-border bg-card p-4 ${className ?? ""}`}>
      <div className="flex items-start justify-between gap-2">
        ...
        <div className="flex shrink-0 items-center gap-1">
          <span className="font-bold text-primary">{formatEGP(order.amount)}</span>
          {isAdmin && onDelete && (
            <Button variant="ghost" size="icon" className="size-8 text-destructive" aria-label="حذف" onClick={onDelete}>
